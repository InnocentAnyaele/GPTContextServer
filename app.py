...
from utils import createIndex, checkExtension, startDeleteThread, queryPineconeIndex, queryIndexWithChromaFromPersistent, createIndexWithChroma
...
def create_app(config = DevelopmentConfig()):
    ...
    def add_data():
        if request.method == 'POST':
            ...
            try:
                ...
                if extension == 'pdf' or 'txt':
                    path_to_pdf = os.path.join(uniqueDirName,fileNamesArray[-1])
                    indexKey = createIndexWithChroma(path_to_pdf)
    ...
    def queryIndex():
        if request.method == 'POST':
            print ('reached the get response index')
            try:
